...
import { AlertCircleIcon, GripVerticalIcon, ImageIcon, UploadIcon, XIcon } from "lucide-react";
import {
  forwardRef,
  useCallback,
  useEffect,
...
async function uploadToCloudinaryViaPresign(
  ...
  const res = await api.post<{ data: Record<string, unknown> }>("/v1/images/upload/presign", {
    folder: keyPrefix,
    resourceType,
  });
...
const MediaUploaderField = forwardRef<MediaUploaderFieldRef, Props>(function MediaUploaderField(
  {
    name,
    label,
    multiple = false,
  ...
  const initialPreviewById = useRef<Record<string, string>>(
    Object.fromEntries(initialFiles.map((f) => [f.id, f.url])),
  );
  ...
  useEffect(() => {
    ...
    for (const f of files) {
      ...
      const fallbackValue =
        f.preview || (f.file instanceof File ? "" : ((f.file as InitialFileMeta).url ?? ""));
      ...
      uploadPendingFiles: async () => {
        ...
        for (const fileEntry of filesToProcess) {
          ...
          if (fileEntry.file instanceof File) {
            const hasRemoteUrl =
              !!idToUrl.current[id] && /^(https?:\/\/)/.test(idToUrl.current[id]);
          ...
          const resolvedUrl =
            ...
            (fileEntry.file instanceof File
              ? fileEntry.preview || ""
              : ((fileEntry.file as InitialFileMeta).url ?? ""));
                    ...
                    <div className="flex gap-2 mt-4">
                      ...
                      {sortedSelectableMedia.length > 0 && (
                        <Button
                          type="button"
                          variant="outline"
                          onClick={() => setShowSelectableMedia(true)}
                        >
                ...
                <div className="border border-dashed rounded-xl p-4 mt-2">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => setShowSelectableMedia(false)}
                    >
